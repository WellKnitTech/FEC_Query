...
def extract_date_from_raw_data(raw_data: Optional[Dict[str, Any]]) -> Optional[datetime]:
    ...
    This function checks multiple common date field names and formats used in FEC bulk data:
    - TRANSACTION_DT (often in MMDDYYYY format - 8 digits)
    ...
    # Log available keys for debugging (use WARNING so we can see it)
    available_keys = list(raw_data.keys())
    logger.warning(f"extract_date_from_raw_data: Available keys in raw_data (first 30): {available_keys[:30]}")
    
    # List of date field names to check first (in order of preference)
    ...
    logger.warning(f"extract_date_from_raw_data: Priority date fields found: {[f for f in priority_date_fields if f in available_keys]}")
    logger.warning(f"extract_date_from_raw_data: Additional date fields found: {additional_date_fields[:10]}")
    ...
    for key in keys_to_log:
        ...
        logger.warning(f"extract_date_from_raw_data: Key '{key}' = {repr(value)} (type: {type(value).__name__})")
    ...
    logger.warning(f"extract_date_from_raw_data: Will check these date fields (in order): {fields_to_check[:10]}")
    ...
    for field_name in fields_to_check:
        ...
        # Log even if the value is falsy, so we can see what's happening
        if date_value is None:
            logger.warning(f"extract_date_from_raw_data: Field '{field_name}' exists but value is None")
            continue
        elif date_value == "":
            logger.warning(f"extract_date_from_raw_data: Field '{field_name}' exists but value is empty string")
            continue
        elif not date_value:
            logger.warning(f"extract_date_from_raw_data: Field '{field_name}' exists but value is falsy: {date_value} (type: {type(date_value).__name__})")
            continue
        
        logger.warning(f"extract_date_from_raw_data: Found date field '{field_name}' with value: {date_value} (type: {type(date_value).__name__})")
        
        try:
            # Handle MMDDYYYY format (8 digits) - common in FEC bulk data
            if isinstance(date_value, str):
                date_str = str(date_value).strip()
                logger.warning(f"extract_date_from_raw_data: Processing string date '{date_str}' (length: {len(date_str)}, repr: {repr(date_str)})")
                
                # MMDDYYYY format (8 digits)
                if len(date_str) == 8 and date_str.isdigit():
                    try:
                        parsed = datetime.strptime(date_str, '%m%d%Y')
                        logger.warning(f"extract_date_from_raw_data: Successfully parsed MMDDYYYY format: {parsed}")
                        return parsed
                    except ValueError as e:
                        logger.warning(f"extract_date_from_raw_data: Failed to parse MMDDYYYY format '{date_str}': {e}")
                        continue
                ...
                # YYYY-MM-DD format (10 characters)
                    ...
                    try:
                        ...
                        logger.warning(f"extract_date_from_raw_data: Successfully parsed YYYY-MM-DD format: {parsed}")
                        return parsed
                    except ValueError as e:
                        logger.warning(f"extract_date_from_raw_data: Failed to parse YYYY-MM-DD format '{date_str}': {e}")
                        continue
                ...
                # ISO format with time
                    ...
                    try:
                        ...
                        logger.warning(f"extract_date_from_raw_data: Successfully parsed ISO format: {parsed}")
                        return parsed
                    except ValueError as e:
                        logger.warning(f"extract_date_from_raw_data: Failed to parse ISO format '{date_str}': {e}")
                        continue
                ...
                # Try parsing as ISO format
                    ...
                    try:
                        ...
                        logger.warning(f"extract_date_from_raw_data: Successfully parsed as ISO: {parsed}")
                        return parsed
                    except ValueError as e:
                        logger.warning(f"extract_date_from_raw_data: Failed to parse as ISO '{date_str}': {e}")
                        continue
    ...
    logger.warning(f"extract_date_from_raw_data: No valid date found in raw_data. Checked {len(fields_to_check)} fields: {fields_to_check[:20]}")
    logger.warning(f"extract_date_from_raw_data: All available keys were: {available_keys}")
    return None
