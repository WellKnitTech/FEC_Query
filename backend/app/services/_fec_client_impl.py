...
# Import from package to use refactored modules
from app.services.fec_client.api_client import APIClient
from app.services.fec_client.cache import CacheManager
from app.services.fec_client.rate_limiter import RateLimiter
from app.services.fec_client.storage import StorageManager
...
class FECClient:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        self.rate_limit_delay = self.rate_limiter.rate_limit_delay
        self.rate_limit_retry_delay = self.rate_limiter.rate_limit_retry_delay
        self.cache_ttls = cache_ttls
        self._in_flight_requests = self.api_client._in_flight_requests
    ...
    def _get_semaphore(self):
        ...
        return self.rate_limiter.get_semaphore()
    ...
    def _generate_cache_key(self, endpoint: str, params: Dict) -> str:
        ...
        return self.cache_manager.generate_cache_key(endpoint, params)
    
    async def _get_from_cache(self, cache_key: str) -> Optional[Dict]:
        """Retrieve data from cache if not expired"""
        return await self.cache_manager.get_from_cache(cache_key)
    
    async def _save_to_cache(self, cache_key: str, data: Dict, ttl_hours: int = 24):
        """Save response to cache"""
        await self.cache_manager.save_to_cache(cache_key, data, ttl_hours)
    
    def _get_cache_ttl(self, endpoint: str) -> int:
        """Get appropriate cache TTL for endpoint type"""
        return self.cache_manager.get_cache_ttl(endpoint)
    ...
    async def _refresh_cache_in_background(
        ...
        # This is handled by the API client module
        pass
    ...
    async def _handle_pagination(
        ...
        """Handle pagination for API responses - delegated to API client"""
        return await self.api_client._handle_pagination(endpoint, params, initial_data, max_results)
    ...
    def _extract_candidate_contact_info(self, candidate_data: Dict) -> Dict:
        """Extract contact information from candidate API response - delegated to storage manager"""
    ...
    def _extract_committee_contact_info(self, committee_data: Dict) -> Dict:
        """Extract contact information from committee API response - delegated to storage manager"""
